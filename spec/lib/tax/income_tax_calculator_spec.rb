  ...
  describe "#calculate" do
    ...
    context "when income is within higher tax brackets" do
      ...
      it "calculates correct tax values for Alberta (AB) with 160,000 income" do
        ...
        expect(result[:total_tax]).to be_within(1).of(43_657)  # Updated
        expect(result[:take_home]).to be_within(1).of(116_343) # Updated
        expect(result[:provincial_tax]).to be_within(1).of(13_943) # Updated
        expect(result[:federal_tax]).to be_within(1).of(29_714) # Updated
      end
    end
